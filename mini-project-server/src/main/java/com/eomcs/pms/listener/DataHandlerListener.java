...
public class DataHandlerListener implements ApplicationContextListener {
  ...
  File itemFile = new File("./item.json");
  ...
  public void contextInitialized(Map<String,Object> context) {
    ...
    loadData(itemList, itemFile, Item[].class);
    loadData(playerList, playerFile, Player[].class);
    ...
    context.put("playerList", playerList);
  }
  ...
  public void contextDestroyed(Map<String,Object> context) {
    ...
    saveData(itemList, itemFile);
    saveData(playerList, playerFile);
  }
  ...
  private <T> void loadData(
    ...
    try {
      ...
      System.out.printf("'%s' 구성 중 오류 발생! - %s\n",
  ...
  private void saveData(Collection<?> list, File file) {
    ...
    try {
      ...
      System.out.printf("총 %d 개의 데이터를 '%s' 에 저장했습니다.\n",
          list.size(), file.getName());

    } catch (IOException e) {
      System.out.printf("데이터를 '%s' 에 저장하는 중 오류 발생! - %s\n",
          file.getName(), e.getMessage());
